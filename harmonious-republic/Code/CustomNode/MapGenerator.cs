...
public partial class MapGenerator : Node2D
{
    ...
    // 将生成的地图数据输出到Data节点
    private void OutPutMap(Block[,,] gameMap)
    {
        // 检查地图数据是否有效
        if (gameMap == null)
        {
    ...
    // 渲染地图图像
    private void RenderMap(Block[,] map, Image environmentImage, int plateSize)
    {
        // 参数验证
        if (map == null || environmentImage == null || plateSize <= 0)
        {
            return;
        }
        ...
        for (int i = 0; i < map.GetLength(0); i++)
        {
            ...
            {
                // 检查数组边界和元素有效性
                if (i >= map.GetLength(0) || j >= map.GetLength(1) || map[i, j] == null)
                    continue;

                // 调整Z值范围，将20-80分成5份，低于20的视为20，高于80的视为80
                int adjustedZ = map[i, j].position.Z;
                if (adjustedZ < HeightLevelMin) adjustedZ = HeightLevelMin;
                if (adjustedZ > HeightLevelMax) adjustedZ = HeightLevelMax;

                // 根据材质和高度确定颜色
                Color color = GetBlockColorByMaterialAndHeight(map[i, j].material, adjustedZ);
    ...
    // 生成地图数据
    private Block[,,] GenerateMap(int plateSize)
    {
        try
        {
            // 初始化环境地图和游戏地图
    ...
    private EnumMaterial DetermineMaterialByConditions(int height, float temperature, float humidity)
    {
        ...
        if (height <= SeaLevel)
        {
            ...
            {
                if (temperature is <= IceTempThresholdLow and >= IceTempThresholdHigh)
                {
        ...
        if (height <= LowLandUpperBound)
        {
            ...
            if (humidity <= LowlandMediumHumidityThreshold)
            {
                if (temperature <= ColdTempThreshold)
                {
                    return EnumMaterial.Stone;
                }
                if (temperature <= CoolTempThreshold)
                {
                    return EnumMaterial.Soil;
                }
                if (temperature <= WarmTempThreshold)
                {
    ...
    // 打印地图高度统计信息
    private void PrintHeightDebugInfo(Block[,] map)
    {
        // 参数验证
        if (map == null)
        {
